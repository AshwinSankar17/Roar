...
from roar.collections.asr.parts.preprocessing.pertub import NoisePerturbation
    ...
    for snr in snrs:
        ...
        with open(input_manifest, "r") as inf, open(out_mfst, "w") as outf:
            for line in inf:
                ...
                row["audio_filepath"] = os.path.join(
                    out_dir, os.path.basename(row["audio_filepath"])
                )
                if save_noise:
                    file_ext = os.path.splitext(row["audio_filepath"])[1]
                    noise_filename = os.path.basename(row["audio_filepath"]).replace(
                        file_ext, "_noise" + file_ext
                    )
...
def process_row(row):
    ...
    for snr in row["snrs"]:
        ...
        perturber = NoisePerturbation(
            manifest_path=row["noise_manifest"],
...
def main():
    ...
    parser.add_argument(
        "--noise_manifest", type=str, required=True, help="path to noise manifest file"
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        required=True,
        help="destination directory for audio and manifests",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="save the noise added to the input signal",
    )
