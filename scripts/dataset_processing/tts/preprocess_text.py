...
$ python <roar_root_path>/scripts/dataset_processing/tts/preprocess_text.py \
    --input_manifest="<data_root_path>/manifest.json" \
    --output_manifest="<data_root_path>/manifest_processed.json" \
    --normalizer_config_path="<roar_root_path>/examples/tts/conf/text/normalizer_en.yaml" \
    --lower_case=True \
    --num_workers=4 \
    --batch_size=16
"""
...
from nemo_text_processing.text_normalization.normalize import (
    Normalizer,
)  # TODO: Change to indic text normalizer
...
def get_args():
    ...
    parser.add_argument(
        ...
        required=False,
        type=Path,
        help="Path to config file for roar_text_processing.text_normalization.normalize.Normalizer.",
    )
    parser.add_argument(
        "--num_workers",
        default=1,
