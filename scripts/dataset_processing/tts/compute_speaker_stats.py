...
$ python <roar_root_path>/scripts/dataset_processing/tts/compute_speaker_stats.py \
...
from roar.collections.tts.parts.utils.tts_dataset_utils import get_base_dir
from roar.collections.tts.torch.tts_data_types import Pitch
...
def _get_sup_data_filepath(
    manifest_entry: dict, audio_dir: Path, sup_data_dir: Path
) -> Path:
    """
...
def main():
    ...
    for entry in tqdm(entries):
        pitch_path = _get_sup_data_filepath(
            manifest_entry=entry, audio_dir=base_dir, sup_data_dir=pitch_data_path
        )
    ...
    pitch_stats = {
        "default": {"pitch_mean": global_pitch_mean, "pitch_std": global_pitch_std}
    }
