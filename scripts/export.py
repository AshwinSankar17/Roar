...
def get_args(argv):
    parser = argparse.ArgumentParser(
        ...
        description=f"Export Roar models to ONNX/Torchscript",
    )
    parser.add_argument("source", help="Source .roar file")
    ...
    parser.add_argument(
        "--cache_support",
        action="store_true",
        help="enables caching inputs for the models support it.",
    )
    parser.add_argument("--device", default="cuda", help="Device to export for")
    parser.add_argument(
        "--check-tolerance", type=float, default=0.01, help="tolerance for verification"
    )
    parser.add_argument(
        "--export-config",
        metavar="KEY=VALUE",
...
def nemo_export(argv):
    ...
    """Convert a .roar saved model into .riva Riva input format."""
    nemo_in = args.source
    out = args.out
    ...
    try:
        with torch.inference_mode():
            # Restore instance from .roar file using generic model restore_from
    ...
    if not isinstance(model, Exportable):
        logging.error(
            "Your NeMo model class ({}) is not Exportable.".format(
