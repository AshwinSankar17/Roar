...
import roar
from roar.core.connectors.save_restore_connector import SaveRestoreConnector #TODO: Fix connectors
from roar.core.neural_types import NeuralType, NeuralTypeComparisonResult
from roar.utils import logging
from roar.utils.cloud import maybe_download_from_cloud
from roar.utils.data_utils import resolve_cache_dir
from roar.utils.model_utils import import_class_by_path, maybe_update_config_version
...
class FileIO(ABC):
    def save_to(self, save_path: str):
        ...
        Implemented via :meth:`roar.core.connectors.save_restore_connector.SaveRestoreConnector.save_to`.
    ...
    def restore_from(
        ...
        """
        Restores model instance (weights and configuration) from a .roar file
    ...
    def from_config_file(cls, path2yaml_file: str):
        """
        Instantiates an instance of Roar Model from YAML config file.
...
class Model(Typing, Serialization, FileIO):
    """
    Abstract class offering interface which should be implemented by all Roar models.
    """
    ...
    def search_huggingface_models(
        ...
        Args:
            model_filter: Optional ModelFilter or List[ModelFilter] (from Hugging Face Hub)
                ...
                Users can then use `model_card.modelId` in `from_pretrained()` to restore a Roar Model.
        ...
        # Inject `roar` library filter
        for mfilter in model_filter:
            if isinstance(mfilter.library, str) and mfilter.library != 'roar':
                logging.warning(f"Model filter's `library` tag updated be `roar`. Original value: {mfilter.library}")
                mfilter.library = "roar"

            elif isinstance(mfilter, Iterable) and 'roar' not in mfilter.library:
                logging.warning(
                    f"Model filter's `library` list updated to include `roar`. Original value: {mfilter.library}"
                )
                mfilter.library = list(mfilter)
                mfilter.library.append('roar')
    ...
    def get_hf_model_filter(cls) -> ModelFilter:
        ...
        """
        model_filter = ModelFilter(library='roar')
    ...
    def from_pretrained(
        ...
        """
        Instantiates an instance of Roar from NVIDIA NGC cloud
        Use restore_from() to instantiate from a local .roar file.
        Args:
        ...
        # HF Hub source
        if '/' in model_name:
            class_, roar_model_file_in_cache = cls._get_hf_hub_pretrained_model_info(
                model_name=model_name, refresh_cache=refresh_cache
            )
        else:
            # NGC source
            class_, roar_model_file_in_cache = cls._get_ngc_pretrained_model_info(
                model_name=model_name, refresh_cache=refresh_cache
            )

        instance = class_.restore_from(
            restore_path=roar_model_file_in_cache,
    ...
    def _get_ngc_pretrained_model_info(cls, model_name: str, refresh_cache: bool = False) -> Tuple[type, str]:
        ...
        Returns:
            ...
            -   The path to the Roar model (.roar file) in some cached directory.
        """
        ...
        roar_model_file_in_cache = maybe_download_from_cloud(
        ...
        return class_, roar_model_file_in_cache
    ...
    def _get_hf_hub_pretrained_model_info(cls, model_name: str, refresh_cache: bool = False) -> Tuple[type, str]:
        ...
        Note:
            ...
            This allows public, externally contributed models to be run freely using Nvidia Roar.
        ...
        Returns:
            A tuple of details describing :
            -   The resolved class of the model. Since the source is external to Roar, always default to using
                the calling class. Depend on target class resolution by restore_from() for calling the correct class.
            -   The path to the Roar model (.roar file) in some cached directory (managed by HF Hub).
        """
        # Resolve the model name without origin for filename
        resolved_model_filename = model_name.split("/")[-1] + '.roar'
        ...
        path = hf_hub_download(
            ...
            library_name="roar",
            library_version=roar.__version__,
...
class typecheck:
    ...
    Requires that the class inherit from :class:`~roar.core.Typing` in order to perform
    ...
    def __call__(self, wrapped, instance: Typing, args, kwargs):
        """
        Wrapper method that can be used on any function of a class that implements :class:`~roar.core.Typing`.
        ...
        """
        if instance is None:
            raise RuntimeError("Only classes which inherit roar.core.Typing can use this decorator !")
