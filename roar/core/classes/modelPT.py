...
from roar import package_info
from roar.core import optim
from roar.core.classes.common import Model
from roar.core.connectors.save_restore_connector import SaveRestoreConnector
from roar.core.optim import prepare_lr_scheduler
...
class ModelPT(LightningModule, Model):
    """
    Interface for Pytorch-lightning based Roar models
    """

    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        """
        Base class from which all Roar models should inherit
        ...
        # init mapping submodule attribute -> config_field for nested Roar models
        self._roar_submodule_name_to_config_field = dict()
    ...
    def register_artifact(
        ...
        if Path(src).suffix == ".roar":
            raise RoarBaseException(
                "Registering .roar files as artifacts not supported. "
    ...
    def has_roar_submodules(self) -> bool:
    ...
    def register_roar_submodule(
        ...
        Args:
            ...
            model: Roar model, instance of ModelPT
        """
        # check it is a real Roar model
        ...
        if not hasattr(self, "_roar_submodule_name_to_config_field"):
            raise RoarBaseException(
        ...
        self._roar_submodule_name_to_config_field[name] = config_field

    def named_roar_modules(
        self, prefix_name: str = "", prefix_config: str = ""
    ) -> Iterator[Tuple[str, str, "ModelPT"]]:
        """
        Returns an iterator over all Roar submodules recursively, yielding
        ...
        if not hasattr(self, "_roar_submodule_name_to_config_field"):
            raise RoarBaseException(
                "Model is not fully initialized. Calling `named_roar_modules` before __init__ not allowed. "
                "Did you forget to call `super().__init__`?"
            )
    ...
    def restore_from(
        ...
        """
        Restores model instance (weights and configuration) from .roar file.
    ...
    def maybe_init_from_pretrained_checkpoint(
        ...
        Initializations:
            ...
            init_from_pretrained_model: Str name of a pretrained model checkpoint (obtained via cloud).
                ...
                with the same format as for init_from_roar_model, except with "name" instead of "path"
            ...
            init_from_ptl_ckpt: Str name of a Pytorch Lightning checkpoint file. It will be loaded and
                ...
                with the same format as for init_from_roar_model.
