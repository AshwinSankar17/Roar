...
from roar.core import classes as roar_classes  # to avoid circular import do not import ModelPT directly
from roar.utils import logging, model_utils
from roar.utils.app_state import AppState
from roar.utils.get_rank import is_global_rank_zero
from roar.utils.model_utils import inject_model_parallel_rank

#TODO: This needs big changes, this is how .nemo files for model checkpoints are saved.
class SaveRestoreConnector:
    ...
    def save_to(self, model: "roar_classes.ModelPT", save_path: str):
        """
        ...
        if is_global_rank_zero():
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if model.has_native_or_submodules_artifacts():
                    self._handle_artifacts(model, roar_file_folder=tmpdir)
                ...
                self._make_roar_file_from_folder(filename=save_path, source_dir=tmpdir)
        else:
            return
    ...
    def load_config_and_state_dict(
        ...
        """
        Restores model instance (weights and configuration) into .roar file

        Args:
            restore_path: path to .roar file from which model should be instantiated
        ...
        Example:
            ```
            model = roar.collections.asr.models.EncDecCTCModel.restore_from('asr.roar')
            assert isinstance(model, roar.collections.asr.models.EncDecCTCModel)
            ```
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                if self.model_extracted_dir is not None and os.path.isdir(self.model_extracted_dir):
                    # Log that Roar will use the provided `model_extracted_dir`
                    logging.info(
                    ...
                    # Extract the roar file into the temporary directory
                    self._unpack_roar_file(
    ...
    def extract_state_dict_from(self, restore_path: str, save_dir: str, split_by_module: bool = False):
        """
        Extract the state dict(s) from a provided .roar tarfile and save it to a directory.

        Args:
            restore_path: path to .roar file from which state dict(s) should be extracted
        ...
        Example:
            To convert the .roar tarfile into a single Model level PyTorch checkpoint
            ::
            state_dict = roar.collections.asr.models.EncDecCTCModel.extract_state_dict_from('asr.roar', './asr_ckpts')


            To restore a model from a Model level checkpoint
            ::
            model = roar.collections.asr.models.EncDecCTCModel(cfg)  # or any other method of restoration
            model.load_state_dict(torch.load("./asr_ckpts/model_weights.ckpt"))


            To convert the .roar tarfile into multiple Module level PyTorch checkpoints
            ::
            state_dict = roar.collections.asr.models.EncDecCTCModel.extract_state_dict_from('asr.roar', './asr_ckpts', split_by_module=True)
        ...
        Returns:
            The state dict that was loaded from the original .roar checkpoint
        """
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                self._unpack_roar_file(path2file=restore_path, out_folder=tmpdir)
                os.chdir(tmpdir)
                model_weights = os.path.join(tmpdir, self.model_weights_ckpt)
    ...
    def register_artifact(self, model, config_path: str, src: str, verify_src_exists: bool = True):
        ...
        if app_state.roar_file_folder is not None:
            src_obj_path = os.path.abspath(os.path.join(app_state.roar_file_folder, src_obj_name))
        else:
        ...
        # this is the case when artifact must be retried from the roar file
        # we are assuming that the location of the right roar file is available from _MODEL_RESTORE_PATH
        elif src.startswith("roar:"):
            return_path = os.path.abspath(os.path.join(app_state.roar_file_folder, src[5:]))
        ...
        # backward compatibility implementation
            ...
            if verify_src_exists:
                raise FileNotFoundError(
                    f"src path does not exist or it is not a path in roar file. src value I got was: {src}. Absolute: {os.path.abspath(src)}"
                )
            else:
    ...
    def _handle_artifacts(self, model, roar_file_folder):
        tarfile_artifacts = []
        app_state = AppState()

        # aggregate artifacts from self and all children recursively
        artifacts_containers = []
        for _, config_path, module in model.named_roar_modules():
            if module.has_artifacts():  # Roar model with artifacts
        ...
        for config_path, artifacts in artifacts_containers:
            for subconf_path, artiitem in artifacts.items():
                ...
                if artiitem.path_type == model_utils.ArtifactPathType.LOCAL_PATH:
                    ...
                    # Generate new uniq artifact name and copy it to roar_file_folder
                    ...
                    shutil.copy2(artiitem.path, os.path.join(roar_file_folder, artifact_uniq_name))

                    # Update artifacts registry
                    artiitem.hashed_path = "roar:" + artifact_uniq_name
                    ...
                    raise ValueError(f"Directly referencing artifacts from other roar files isn't supported yet")
        ...
        for module in model.modules():
            if isinstance(module, roar_classes.ModelPT):  # if Roar model
        ...
        if len(tarfile_artifacts) > 0 and len(restoration_paths) > 0:
            # Need to step into roar archive to extract file
            # Get path where the command is executed - the artifacts will be "retrieved" there
            # (original .roar behavior)
            cwd = os.getcwd()
            # Step into the roar archive to try and find the file
            # TemporaryDirectory context must always be outer to try-catch chdir otherwise it crashes on Windows
            with tempfile.TemporaryDirectory() as archive_dir:
                try:
                    # unpack all restorations paths (roar checkpoints)
                    # in roar checkpoints all resources contain hash in name, so there should be no collisions
                    for path in restoration_paths:
                        if self.model_extracted_dir:
                            shutil.copytree(src=path, dst=archive_dir, dirs_exist_ok=True)
                        else:
                            self._unpack_roar_file(path2file=path, out_folder=archive_dir)
                    os.chdir(archive_dir)
                    for conf_path, artiitem in tarfile_artifacts:
                        # Get basename and copy it to roar_file_folder
                        if 'roar:' in artiitem.path:
                            artifact_base_name = artiitem.path.split('roar:')[1]
                        else:
                            artifact_base_name = os.path.basename(artiitem.path)
                        # no need to hash here as we are in tarfile_artifacts which are already hashed
                        artifact_uniq_name = artifact_base_name
                        shutil.copy2(artifact_base_name, os.path.join(roar_file_folder, artifact_uniq_name))

                        # Update artifacts registry
                        new_artiitem = model_utils.ArtifactItem()
                        new_artiitem.path = "roar:" + artifact_uniq_name
    ...
    def _update_subconfigs(model: "roar_classes.ModelPT", path2yaml_file):
        ...
        for _, conf_path, submodule in model.named_roar_modules():
    ...
    @staticmethod
    def _make_roar_file_from_folder(filename, source_dir):
    ...
    @staticmethod
    def _unpack_roar_file(path2file: str, out_folder: str, extract_config_only: bool = False) -> str:
