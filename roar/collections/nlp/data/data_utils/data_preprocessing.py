...
from roar.utils.env_var_parsing import get_envint
...
def get_multi_label_stats(labels, outfile="stats.tsv", verbose=True):
    ...
    if verbose:
        three_most_frequent_classes = sorted(
            class_count_dict, key=lambda idx: class_count_dict[idx][1], reverse=True
        )
...
def dataset_to_ids(
    ...
    if use_cache and os.path.isfile(cached_ids_dataset):
        ...
        if cache_ids and (
            ...
            or (cache_data_per_node and get_envint("LOCAL_RANK", 0) == 0)
        ):
