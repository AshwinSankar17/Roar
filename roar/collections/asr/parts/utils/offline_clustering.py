...
def cos_similarity(
    emb_a: torch.Tensor, emb_b: torch.Tensor, eps=torch.tensor(3.5e-4)
) -> torch.Tensor:
    """
...
def kmeans_torch(
    ...
    for iter_count in range(iter_limit):
        ...
        for index in range(num_clusters):
            selected_cluster = (
                torch.nonzero(selected_cluster_indices == index).squeeze().to(device)
            )
...
def getTheLargestComponent(
    affinity_mat: torch.Tensor, seg_index: int, device: torch.device
) -> torch.Tensor:
    """
...
def isGraphFullyConnected(
    affinity_mat: torch.Tensor, device: torch.device
) -> torch.Tensor:
    """
    Check whether the given affinity matrix is a fully connected graph.
    """
    return (
        getTheLargestComponent(affinity_mat, 0, device).sum() == affinity_mat.shape[0]
    )


def getKneighborsConnections(
    affinity_mat: torch.Tensor, p_value: int, mask_method: str = "binary"
) -> torch.Tensor:
    """
    ...
    if mask_method == "binary" or mask_method is None:
        ...
        binarized_affinity_mat[indices_row, indices_col] = affinity_mat[
            indices_row, indices_col
        ].half()
    elif mask_method == "sigmoid":
        binarized_affinity_mat[indices_row, indices_col] = torch.sigmoid(
            affinity_mat[indices_row, indices_col]
        ).half()
    else:
...
def getAffinityGraphMat(affinity_mat_raw: torch.Tensor, p_value: int) -> torch.Tensor:
    ...
    X = (
        affinity_mat_raw
        if p_value <= 0
        else getKneighborsConnections(affinity_mat_raw, p_value)
    )
...
def getRepeatedList(
    mapping_argmat: torch.Tensor, score_mat_size: torch.Tensor
) -> torch.Tensor:
    """
    ...
    """
    repeat_list = torch.zeros(score_mat_size, dtype=torch.int32).to(
        mapping_argmat.device
    )
...
def get_scale_interpolated_embs(
    ...
    for scale_idx in scale_list:
        ...
        repeat_list = getRepeatedList(mapping_argmat, torch.tensor(emb_t.shape[0])).to(
            device
        )
...
def addAnchorEmb(
    emb: torch.Tensor, anchor_sample_n: int, anchor_spk_n: int, sigma: float
) -> torch.Tensor:
    """
...
def getEnhancedSpeakerCount(
    ...
    comp_est_num_of_spk = torch.tensor(
        max(torch.mode(torch.tensor(est_num_of_spk_list))[0].item() - anchor_spk_n, 1)
    )
...
def split_input_data(
    ...
    embeddings_in_scales, timestamps_in_scales = list(embeddings_in_scales), list(
...
class SpectralClustering:
    ...
    def getSpectralEmbeddings(
        self, affinity_mat: torch.Tensor, n_spks: int = 8, cuda: bool = False
    ) -> torch.Tensor:
        """
        ...
        """
        laplacian = getLaplacian(affinity_mat)
        _, diffusion_map_ = eigDecompose(
            laplacian, cuda=cuda, device=affinity_mat.device
        )
