...
def select_channels(
    signal: npt.NDArray, channel_selector: Optional[ChannelSelectorType] = None
) -> npt.NDArray:
    """
    ...
    if signal.ndim == 1:
        ...
        if channel_selector not in [None, 0, "average"]:
            raise ValueError(
                "Input signal is one-dimensional, channel selector (%s) cannot not be used.",
                str(channel_selector),
            )
        return signal
    ...
    if channel_selector is None:
        ...
        if channel_selector >= num_channels:
            raise ValueError(
                f"Cannot select channel {channel_selector} from a signal with {num_channels} channels."
            )
...
def theoretical_coherence(
    ...
    for p in range(num_mics):
        ...
        for q in range(p + 1, num_mics):
            ...
            if field == "spherical":
                desired_coherence[:, p, q] = sinc_unnormalized(
                    angular_freq * dist_pq / sound_velocity
                )
            else:
...
def estimated_coherence(S: npt.NDArray, eps: float = 1e-16) -> npt.NDArray:
    ...
    estimated_coherence = np.zeros(
        (num_subbands, num_channels, num_channels), dtype=complex
    )
    ...
    for p in range(num_channels):
        ...
        for q in range(p + 1, num_channels):
            ...
            estimated_coherence[:, p, q] = cross_psd / np.sqrt(
                psd[:, p] * psd[:, q] + eps
            )
            # symmetry
...
def generate_approximate_noise_field(
    ...
    return transform_to_match_coherence(
        signal=noise_signal, desired_coherence=desired_coherence, method=method
    )
...
def convmtx_numpy(
    x: np.ndarray, filter_length: int, delay: int = 0, n_steps: Optional[int] = None
) -> np.ndarray:
    ...
    if x.ndim != 1:
        raise ValueError(
            f"Expecting one-dimensional signal. Received signal with shape {x.shape}"
        )
    ...
    return scipy.linalg.toeplitz(
        x_pad, np.hstack([x_pad[0], np.zeros(filter_length - 1)])
    )


def convmtx_mc_numpy(
    x: np.ndarray, filter_length: int, delay: int = 0, n_steps: Optional[int] = None
) -> np.ndarray:
    ...
    if x.ndim != 2:
        raise ValueError(
            f"Expecting two-dimensional signal. Received signal with shape {x.shape}"
        )
    ...
    for m in range(x.shape[1]):
        mc_mtx.append(
            convmtx_numpy(
                x[:, m], filter_length=filter_length, delay=delay, n_steps=n_steps
            )
        )
...
def scale_invariant_target_numpy(
    estimate: np.ndarray, target: np.ndarray, eps: float = 1e-10
) -> np.ndarray:
