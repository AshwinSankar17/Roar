...
from roar.collections.asr.data.audio_to_text import ASRManifestProcessor, expand_sharded_filepaths
...
class _AudioTextDALIDataset(Iterator):
    ...
    def __init__(
        ...
        if has_preprocessor:
            if preprocessor_cfg._target_ == "roar.collections.asr.modules.AudioToMelSpectrogramPreprocessor":
                feature_type = "mel_spectrogram"
            elif preprocessor_cfg._target_ == "roar.collections.asr.modules.AudioToMFCCPreprocessor":
                feature_type = "mfcc"
            else:
                raise ValueError(
        ...
        with self.pipe:
            if audio_tar_filepaths is None and audio_tar_index_filepaths is None:
                audio, indices = dali.fn.readers.roar_asr(
                    name="Reader",
        ...
        y = time.time()
        logging.info(f"Time to build roar manifest processor - {(y - x)} seconds")
