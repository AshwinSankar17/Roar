...
from roar.collections.asr.parts.preprocessing.features import make_seq_mask_like
from roar.collections.asr.parts.utils.audio_utils import toeplitz
from roar.core.classes import Loss, Typing, typecheck
...
def scale_invariant_target(
    ...
    estimate_dot_target = temporal_mean(
        estimate * target, mask=mask, keepdim=True, eps=eps
    )
...
def calculate_sdr_batch(
    ...
    if scale_invariant and convolution_invariant:
        raise ValueError(
            f"Arguments scale_invariant and convolution_invariant cannot be used simultaneously."
        )
