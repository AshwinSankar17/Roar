...
from roar.collections.asr.models import EncDecClassificationModel
from roar.collections.tts.parts.utils.tts_dataset_utils import normalize_volume
...
class AudioTrimmer(ABC):
    ...
    @abstractmethod
    def trim_audio(
        self, audio: np.array, sample_rate: int, audio_id: str
...
class EnergyAudioTrimmer(AudioTrimmer):
    def __init__(
        ...
        Args:
            db_threshold: Audio frames at least db_threshold decibels below ref_amplitude will be considered silence.
    ...
    def trim_audio(
        ...
        start_sample = librosa.core.frames_to_samples(
            start_frame, hop_length=self.trim_hop_length
        )
        end_sample = librosa.core.frames_to_samples(
            end_frame, hop_length=self.trim_hop_length
        )
...
class VadAudioTrimmer(AudioTrimmer):
    def __init__(
        ...
        self.device = device
        self.vad_model = (
            EncDecClassificationModel.from_pretrained(model_name=model_name)
            .eval()
            .to(self.device)
        )
    ...
    def _detect_speech(self, audio: np.array) -> np.array:
        ...
        audio_signal = torch.tensor(
            audio_frames, dtype=torch.float32, device=self.device
        )
        # [1]
        audio_signal_len = torch.tensor(
            audio_frame_lengths, dtype=torch.int32, device=self.device
        )
    ...
    def _scale_sample_indices(
        self, start_sample: int, end_sample: int, sample_rate: int
    ) -> Tuple[int, int]:
    ...
    def trim_audio(
        ...
        if sample_rate == self.vad_sample_rate:
            ...
            vad_audio = librosa.resample(
                audio, orig_sr=sample_rate, target_sr=self.vad_sample_rate
            )
