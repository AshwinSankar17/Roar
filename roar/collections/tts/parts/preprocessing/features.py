...
from roar.collections.tts.parts.utils.audio_preprocessing import (
...
from roar.collections.tts.parts.preprocessing.pertub import AudioAugmentor
...
def _get_feature_filepath(
    ...
    feature_filepath = (
        feature_dir / feature_name / audio_filepath_rel.with_suffix(".pt")
    )
...
def _collate_feature(
    feature_dict: Dict[str, Tensor],
    feature_name: Optional[str],
    train_batch: List[Dict[str, Tensor]],
) -> None:
    if feature_name is None:
        return
...
def clean_spectrogram_batch(
    spectrogram: torch.Tensor, spectrogram_len: torch.Tensor, fill_value=0.0
) -> torch.Tensor:
    """
...
class MelSpectrogramFeaturizer:
    ...
    def compute_mel_spec(
        ...
        audio_filepath, _ = get_audio_filepaths(
            manifest_entry=manifest_entry, audio_dir=audio_dir
        )
        ...
        # [1, spec_dim, T_spec]
        spec_tensor, _ = self.preprocessor(
    ...
    def save(
        ...
        spec_tensor = self.compute_mel_spec(
            manifest_entry=manifest_entry, audio_dir=audio_dir
        )
        _save_pt_feature(
...
class EnergyFeaturizer:
    def __init__(
        self, spec_featurizer: MelSpectrogramFeaturizer, feature_name: str = "energy"
    ) -> None:
        self.feature_name = feature_name
    ...
    def compute_energy(self, manifest_entry: Dict[str, Any], audio_dir: Path) -> Tensor:
        ...
        """
        # [1, T_audio]
        spec = self.spec_featurizer.compute_mel_spec(
            manifest_entry=manifest_entry, audio_dir=audio_dir
        )
        # [T_audio]
    ...
    def save(
        ...
        energy_tensor = self.compute_energy(
            manifest_entry=manifest_entry, audio_dir=audio_dir
        )
        _save_pt_feature(
            feature_name=self.feature_name,
