...
class AudioSegment(object):
    ...
    def __str__(self):
        ...
        if self.num_channels == 1:
            ...
            return (
                "%s: num_samples=%d, sample_rate=%d, duration=%.2fsec, num_channels=%d, rms=[%s]"
                % (
                    type(self),
                    self.num_samples,
                    self.sample_rate,
    ...
    def from_file(
        ...
        if (
            ...
            or os.path.splitext(audio_file)[-1] in sf_supported_formats
        ):
            try:
        ...
        if HAVE_PYDUB and samples is None:
            try:
                ...
                logging.error(
                    f"Loading {audio_file} via pydub raised CouldntDecodeError: `{err}`."
                )

        if samples is None:
            libs = "soundfile, and pydub" if HAVE_PYDUB else "soundfile"
            raise Exception(
                f"Your audio file {audio_file} could not be decoded. We tried using {libs}."
            )
        ...
        try:
            with sf.SoundFile(audio_file, "r") as f:
                ...
                if target_sr is not None:
                    n_segments_at_original_sr = math.ceil(
                        n_segments * sample_rate / target_sr
                    )
                else:
    ...
    def pad(self, pad_size, symmetric=False):
        ...
        if samples_ndim == 1:
            ...
            pad_width = (
                ((pad_size, pad_size), (0, 0)) if symmetric else ((0, pad_size), (0, 0))
            )
        else:
            raise NotImplementedError(
