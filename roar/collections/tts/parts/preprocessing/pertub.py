...
try:
    ...
    from roar.utils.exceptions import LightningNotInstalledException

    HAVE_OMEGACONG_WEBDATASET = False


try:
    from roar.collections.tts.parts.utils import numba_utils
...
class NoisePerturbationWithNormalization(Perturbation):
    ...
    def __init__(
        ...
        from roar.collections.asr.data.audio_to_text import RandomizedChainDataset
    ...
    def perturb(self, data):
        if self._duration_sec > data.duration:
            if not self._pad_to_duration:
                raise ValueError(
                    f"audio length < {self._duration_sec} sec and pad_to_duration is set to False"
                )
            start_time = 0.0
...
def process_augmentations(
    ...
    ```python
    from roar.collections.asr.parts import perturb
...
class AugmentationDataset(IterableDataset):
    ...
    def __init__(
        ...
        # import here to avoid circular import error
        from roar.collections.asr.data.audio_to_text import expand_sharded_filepaths
