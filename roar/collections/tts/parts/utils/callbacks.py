...
from roar.collections.tts.parts.utils.helpers import create_plot
...
def _load_vocoder(model_name: Optional[str], checkpoint_path: Optional[str], type: str):
    ...
    if type == "hifigan":
        ...
        from roar.collections.tts.models import UnivNetModel
    ...
    elif checkpoint_path.endswith(".roar"):
...
class LoggingCallback(Callback):
    ...
    def _log_audio(self, audio: AudioArtifact, log_dir: Path, step: int):
        ...
        if self.wandb_logger:
            wandb_audio = (
                wandb.Audio(
                    audio.data, sample_rate=audio.sample_rate, caption=audio.id
                ),
            )
    ...
    def on_fit_start(self, trainer: Trainer, model: LightningModule):
        ...
        for batch_dict in self.data_loader:
            ...
            for generator in self.generators:
                audio, images = generator.generate_artifacts(
                    model=model, batch_dict=batch_dict, initial_log=True
                )
                audio_list += audio
                image_list += images
        ...
        self._log_artifacts(
            audio_list=audio_list, image_list=image_list, log_dir=log_dir
        )
    ...
    def on_train_epoch_end(self, trainer: Trainer, model: LightningModule):
        ...
        for batch_dict in self.data_loader:
            ...
            for generator in self.generators:
                audio, images = generator.generate_artifacts(
                    model=model, batch_dict=batch_dict
                )
                audio_list += audio
                image_list += images
...
class AudioCodecArtifactGenerator(ArtifactGenerator):
    ...
    def _generate_audio(
        self, model, audio_ids, audio, audio_len, save_input: bool = False
    ):
    ...
    def generate_artifacts(
        ...
        image_artifacts = self._generate_images(
            model=model, audio_ids=audio_ids, audio=audio, audio_len=audio_len
        )
...
class FastPitchArtifactGenerator(ArtifactGenerator):
    ...
    def _generate_audio(self, mels, mels_len, hop_length):
        ...
        audio_pred_lens = librosa.core.frames_to_samples(
            mels_len_array, hop_length=hop_length
        )
        return audio_pred, audio_pred_lens

    def _generate_predictions(
        self, model: LightningModule, audio_ids: List[str], batch_dict: Dict
    ):
        audio_artifacts = []
        image_artifacts = []
        ...
        if self.log_audio:
            ...
            audio_pred, audio_pred_lens = self._generate_audio(
                mels=mels_pred,
                mels_len=mels_pred_len,
                hop_length=model.preprocessor.hop_length,
            )
            for i, audio_id in enumerate(audio_ids):
