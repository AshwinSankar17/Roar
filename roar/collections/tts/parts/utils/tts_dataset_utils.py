...
def get_audio_filepaths(
    manifest_entry: Dict[str, Any], audio_dir: Path
) -> Tuple[Path, Path]:
    """
    ...
    audio_filepath_abs, audio_filepath_rel = get_abs_rel_paths(
        input_path=audio_filepath, base_path=audio_dir
    )
...
class BetaBinomialInterpolator:
    ...
    def __init__(self, round_mel_len_to=50, round_text_len_to=10, cache_size=500):
        ...
        self.bank = functools.lru_cache(maxsize=cache_size)(
...
def filter_dataset_by_duration(
    entries: List[Dict[str, Any]], min_duration: float, max_duration: float
):
    """
