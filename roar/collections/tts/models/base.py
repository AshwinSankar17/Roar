...
from roar.collections.tts.parts.utils.helpers import OperationMode
from roar.core.classes import ModelPT
from roar.core.classes.common import PretrainedModelInfo, typecheck
from roar.core.neural_types.elements import AudioSignal
...
class GlowVocoder(Vocoder):
    ...
    roar.collections.asr.parts.FilterbankFeatures"""
    ...
    def nemo_infer(
        self,
    ):  # Prepend with roar to avoid any .infer() clashes with lightning or pytorch
    ...
    def update_bias_spect(self):
        ...
        with self.nemo_infer():
            ...
            bias_audio = self.convert_spectrogram_to_audio(
                spec=spect, sigma=0.0, denoise=False
            )
...
class MelToSpec(ModelPT, ABC):
    ...
    @abstractmethod
    def convert_mel_spectrogram_to_linear(
        self, mel: "torch.tensor", **kwargs
    ) -> "torch.tensor":
        """
...
class TextToWaveform(ModelPT, ABC):
    ...
    def convert_text_to_waveform(
        self, *, tokens: "torch.tensor", **kwargs
    ) -> "List[torch.tensor]":
        """
