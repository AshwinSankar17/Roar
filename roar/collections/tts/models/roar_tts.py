...
class RoarTTSModel(TextToWaveform, Exportable):
    """Roar TTS model that is used to generate audio from text."""
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        spectrogram_resolutions = cfg.spectrogram_resolutions
        mel_dims = cfg.mel_dims

        assert len(spectrogram_resolutions) == len(mel_dims)

        self.mel_loss_fn = MultiResolutionMelLoss(
            sample_rate=cfg.sample_rate,
            resolutions=spectrogram_resolutions,
            mel_dims=mel_dims,
            scale_factor=cfg.get("mel_loss_scale", 1.0),
        )
        ...
        self.msd = MultiResolutionDiscriminator(
            resolutions=spectrogram_resolutions,
            spectral_norm=cfg.discriminator_use_spectral_norm,
            debug=cfg.debug if "debug" in cfg else False,
        )
