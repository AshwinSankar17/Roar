...
from roar.collections.tts.parts.preprocessing.feature_processors import FeatureProcessor
from roar.collections.tts.parts.preprocessing.features import Featurizer
...
class TextToSpeechDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        audio_filepath_abs, audio_filepath_rel = get_abs_rel_paths(
            input_path=audio_filepath, base_path=data.audio_dir
        )
        ...
        if self.include_align_prior:
            ...
            spec_len = 1 + librosa.core.samples_to_frames(
                audio.shape[0], hop_length=self.align_prior_hop_length
            )
            align_prior = beta_binomial_prior_distribution(
                phoneme_count=text_len, mel_count=spec_len
            )
    ...
    def collate_fn(self, batch: List[dict]):
        ...
        batch_tokens = stack_tensors(
            token_list, max_lens=[token_max_len], pad_value=self.text_tokenizer.pad
        )
