...
from roar.core.neural_types.elements import AudioSignal, MelSpectrogramType, VoidType
...
class DiscriminatorR(NeuralModule):
    def __init__(self, resolution, use_spectral_norm):
        super().__init__()
...
class MultiResolutionDiscriminator(NeuralModule):
    def __init__(self, resolutions, debug=False, use_spectral_norm=False):
        super().__init__()
        ...
        self.discriminators = nn.ModuleList([DiscriminatorR(resolution, use_spectral_norm) for resolution in self.resolutions])
