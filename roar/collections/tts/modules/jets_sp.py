...
class PromptEncoder(NeuralModule):
    def __init__(self, n_mel_channels, prompt_backbone):
        super(PromptEncoder, self).__init__()
        self.in_proj = torch.nn.Linear(n_mel_channels, prompt_backbone.n_head * prompt_backbone.d_head)
        self.prompt_backbone = prompt_backbone

    def forward(self, reference_spec, reference_spec_lens):
        # print("LN 102 -> REF SPEC SHAPE : ", reference_spec.shape)
        prompt_emb = self.in_proj(reference_spec)
        prompt_emb, _ = self.prompt_backbone(input=prompt_emb, seq_lens=reference_spec_lens)
...
class TemporalPredictor(NeuralModule):
    ...
    def __init__(
        ...
        for i in range(n_layers):
            ...
            if i % (cross_attn_interval + 1) == 0:
                self.attn_layers.append(torch.nn.MultiheadAttention(
                    filter_size, n_attn_heads, dropout, batch_first=True 
                ))
    ...
    def forward(self, enc, enc_mask, conditioning=None):
        out = enc * enc_mask
        out = out.transpose(1, 2) # after B, D, T
        idx = 0
        for i, layer in enumerate(self.conv_layers):
            out = layer(out, conditioning=conditioning) # o/p B, T, D
            if i % (self.cross_attn_interval + 1) == 0:
                out = out.transpose(1, 2) # B, D, T
                # print("LN 175 -> out shape : ", out.shape, conditioning.shape)
                # print(enc_mask.shape)
                out, _ = self.attn_layers[idx](
...
class JETSModule(NeuralModule, adapter_mixins.AdapterModuleMixin):
    ...
    def get_prompt_embedding(self, reference_spec, reference_spec_lens):
        """spk_emb: Bx1xT1"""
        if self.prompt_encoder is not None:
            prompt_emb = self.prompt_encoder(reference_spec=reference_spec.transpose(1, 2), reference_spec_lens=reference_spec_lens)
        else:
            raise "speaker encoder not found"
            prompt_emb = None
        return prompt_emb
    ...
    def forward(
        ...
        *,
        ...
        prompt_emb = self.get_prompt_embedding(
        ...
        log_durs_predicted = self.duration_predictor(
            enc_out, enc_mask, conditioning=prompt_emb
        )
        ...
        # Predict pitch
        pitch_predicted = self.pitch_predictor(enc_out, enc_mask, conditioning=prompt_emb)
    ...
    def infer(
        ...
        *,
        ...
        if pitch is not None:
            ...
            pitch_pred = self.pitch_predictor(enc_out, enc_mask, conditioning=prompt_emb)
