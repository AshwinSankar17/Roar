    ...
    def __init__(
        ...
        for i in range(n_layers):
            ...
            if i % cross_attn_interval == cross_attn_interval - 1:
                self.attn_layers = MultiHeadCrossAttn(
                    n_attn_heads, filter_size, d_head, 0.2, pre_lnorm=pre_ln
                )
    ...
    def forward(self, enc, enc_mask, conditioning=None):
        ...
        for i, layer in enumerate(self.conv_layers):
            ...
            if i % self.cross_attn_interval == self.cross_attn_interval - 1:
                out = out.transpose(1, 2)
                out = self.attn_layers[idx](
                    out, conditioning, mask=~enc_mask.squeeze(2)
                )
                out = out.transpose(1, 2)
                idx += 1
...
class SpeechPromptEncoder(NeuralModule):
    def __init__(
        ...
        self.out_proj = torch.nn.Linear(prompt_encoder.d_model, out_channels * 2)

    def forward(self, inp, input_lengths):
        out = self.inp_proj(inp)
        out = self.prompt_encoder(out, input_lengths)
        prosody, speaker = self.out_proj(out)
        return prosody, speaker
...
class JETSModule(NeuralModule, adapter_mixins.AdapterModuleMixin):
    ...
    def get_prompt_embedding(self, batch_size, reference_spec, reference_spec_lens):
        """spk_emb: Bx1xD"""
        if self.prompt_encoder is not None:
            prosody_emb, spk_emb = self.prompt_encoder(
                batch_size, reference_spec, reference_spec_lens
            )
        else:
            prosody_emb, spk_emb = None, None

        return prosody_emb, spk_emb
    ...
    def forward(
        ...
        *,
        ...
        prosody_emb, spk_emb = self.get_prompt_embedding(
        ...
        log_durs_predicted = self.duration_predictor(
            enc_out, enc_mask, conditioning=prosody_emb
        )
