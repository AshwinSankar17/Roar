...
from roar.core.classes import Loss
from roar.core.classes.common import typecheck
from roar.core.neural_types import LabelsType, LogitsType, LossType, NeuralType
...
class MultiSimilarityLoss(Loss):
    ...
    def __init__(
        ...
        scale_pos: Optional[
            float
        ] = 2.0,  # Params found to work best in our experiments
    ...
    def forward(self, logits, labels):
        ...
        for i in range(logits.size(0)):
            ...
            positive_sims = positive_sims[
                positive_sims.lt(1 - self._epsilon)
            ]  # omit identical pairs
            ...
            hard_negatives = negative_sims[
                negative_sims.gt(min(positive_sims) - self._margin)
            ]

            # positives that are less similar than the most-similar negative
            hard_positives = positive_sims[
                positive_sims.lt(max(negative_sims) + self._margin)
            ]
            ...
            pos_term = (
                ...
                * torch.log(
                    1
                    + torch.sum(
                        torch.exp(-self._scale_pos * (hard_positives - self._offset))
                    )
                )
            )
            neg_term = (
                ...
                * torch.log(
                    1
                    + torch.sum(
                        torch.exp(self._scale_neg * (hard_negatives - self._offset))
                    )
                )
            )
        ...
        if len(losses) == 0:
            ...
            logging.info(
                f"Encountered zero loss in multisimloss, loss = {loss}. No hard examples found in the batch"
            )
        else:
