...
from roar.collections.common.callbacks import EMA
from roar.constants import ROAR_ENV_VARNAME_TESTING, ROAR_ENV_VARNAME_VERSION
from roar.utils import logging, timers
from roar.utils.app_state import AppState
from roar.utils.callbacks import RoarModelCheckpoint, PreemptionCallback
from roar.utils.env_var_parsing import get_envbool
from roar.utils.exceptions import RoarBaseException
from roar.utils.get_rank import is_global_rank_zero
from roar.utils.lightning_logger_patch import add_filehandlers_to_pl_logger
from roar.utils.loggers import ClearMLLogger, ClearMLParams, DLLogger, DLLoggerParams, MLFlowParams
from roar.utils.model_utils import uninject_model_parallel_rank


class NotFoundError(RoarBaseException):
    """ Raised when a file or folder is not found"""


class LoggerMisconfigurationError(RoarBaseException):
...
class CheckpointMisconfigurationError(RoarBaseException):
...
class CallbackParams:
    ...
    always_save_roar: bool = False
    save_roar_on_train_end: Optional[bool] = True  # Whether to automatically save .roar file durin on_train_end hook
...
def exp_manager(trainer: 'pytorch_lightning.Trainer', cfg: Optional[Union[DictConfig, Dict]] = None) -> Optional[Path]:
    ...
    error_checks(trainer, cfg)  # Ensures that trainer options are compliant with Roar and exp_manager arguments
    ...
    # This is set if the env var ROAR_TESTING is set to True.
    roar_testing = get_envbool(ROAR_ENV_VARNAME_TESTING, False)

    # Handle logging to file
    log_file = log_dir / f'roar_log_globalrank-{global_rank}_localrank-{local_rank}.txt'
    if cfg.log_local_rank_0_only is True and not roar_testing:
        if local_rank == 0:
            logging.add_file_handler(log_file)
    elif cfg.log_global_rank_0_only is True and not roar_testing:
        if global_rank == 0:
            logging.add_file_handler(log_file)
    else:
        # Logs on all ranks.
    ...
    if is_global_rank_zero():
        ...
        logging.add_err_file_handler(log_dir / 'roar_error_log.txt')

        # Add lightning file logging to global_rank zero
        add_filehandlers_to_pl_logger(log_dir / 'lightning_logs.txt', log_dir / 'roar_error_log.txt')
...
def error_checks(trainer: 'pytorch_lightning.Trainer', cfg: Optional[Union[DictConfig, Dict]] = None):
    """
    Checks that the passed trainer is compliant with Roar and exp_manager's passed configuration. Checks that:
    ...
    if trainer.num_nodes > 1 and not check_slurm(trainer):
        logging.error(
            ...
            " Please note that this is not tested in Roar and could result in errors."
        )
    if trainer.num_devices > 1 and not isinstance(trainer.strategy, DDPStrategy):
        logging.error(
            "You are running multi-gpu without ddp.Please note that this is not tested in Roar and could result in "
            "errors."
        )
...
def get_log_dir(
    ...
    # Default exp_dir to ./roar_experiments if None was passed
    _exp_dir = exp_dir
    if exp_dir is None:
        _exp_dir = str(Path.cwd() / 'roar_experiments')
    ...
    # Use user-defined exp_dir, project_name, exp_name, and versioning options
        ...
        version = version or os.environ.get(ROAR_ENV_VARNAME_VERSION, None)
        ...
        if not version:
            if resume_if_exists:
                ...
                os.environ[ROAR_ENV_VARNAME_VERSION] = "" if version is None else version
...
def configure_checkpointing(
    ...
    if params.prefix is None:
        params.prefix = name
    RoarModelCheckpoint.CHECKPOINT_NAME_LAST = params.filename + '-last'
    ...
    checkpoint_callback = RoarModelCheckpoint(n_resume=resume, **params)
...
def clean_exp_ckpt(exp_log_dir: Union[str, Path], remove_ckpt: bool = True, remove_roar: bool = False):
    """
    Helper method that removes Pytorch Lightning .ckpt files or Roar .roar files from the checkpoint directory
    ...
    Args:
        ...
        remove_roar: bool, whether to remove all *.roar files in the checkpoints directory.
    """
    ...
    if remove_roar:
        logging.info("Deleting *.roar files ...")
        roar_files = glob.glob(os.path.join(exp_log_dir, "checkpoints", "*.roar"))
        for filepath in roar_files:
            os.remove(filepath)
            logging.info(f"Deleted file : {filepath}")
