...
from roar.utils.formatters.colors import Fore as ForegroundColors
from roar.utils.formatters.utils import check_color_support, to_unicode

__all__ = ["BaseRoarFormatter"]
...
class BaseRoarFormatter(BaseFormatter):
    DEFAULT_FORMAT = "%(color)s[Roar %(levelname)1.1s %(asctime)s %(module)s:%(lineno)d]%(end_color)s %(message)s"


class DebugRoarFormatter(BaseFormatter):
    DEFAULT_FORMAT = (
        "%(color)s[Roar %(levelname)1.1s %(asctime)s %(module)s:%(lineno)d rank:%(rank)s]%(end_color)s %(message)s"
    )
