...
from roar.utils.model_utils import inject_model_parallel_rank, uninject_model_parallel_rank


class RoarModelCheckpoint(ModelCheckpoint):
    """ Light wrapper around Lightning's ModelCheckpoint to force a saved checkpoint on train_end.
    Extends Lightning's on_save_checkpoint func to save the .roar file. Saves the .roar file based 
    ...
    def __init__(
        ...
        save_roar_on_train_end: bool = True,
        ...
        **kwargs,
        ...
        self.always_save_roar = always_save_roar
        self.save_roar_on_train_end = save_roar_on_train_end
        self.save_best_model = save_best_model
        if self.save_best_model and not self.save_roar_on_train_end:
            logging.warning(
                (
                    "Found save_best_model is True and save_roar_on_train_end is False. "
                    "Set save_roar_on_train_end to True to automatically save the best model."
                )
            )
        ...
        if self.save_top_k != -1 and n_resume:
            ...
            self.roar_topk_check_previous_run()

    def roar_topk_check_previous_run(self):
        try:
            self.best_k_models
            ...
            raise AttributeError("Lightning's ModelCheckpoint was updated. RoarModelCheckpoint will need an update.")
        self.best_k_models = {}
        self.kth_best_model_path = ""
    ...
    def on_save_checkpoint(self, trainer, pl_module, checkpoint):
        ...
        if not self.always_save_roar:
            return output
        # Load the best model and then re-save it
        app_state = AppState()
        if app_state.model_parallel_size is not None and app_state.model_parallel_size > 1:
            logging.warning(f'always_save_roar will slow down training for model_parallel > 1.')
        # since we are creating tarfile artifacts we need to update .roar path
        ...
        if self.save_best_model:
            ...
            logging.info(f"New best .roar model saved to: {app_state.model_restore_path}")
            pl_module.load_state_dict(old_state_dict, strict=True)
        else:
            if torch.distributed.is_initialized():
                torch.distributed.barrier()
            pl_module.save_to(save_path=app_state.model_restore_path)
            logging.info(f"New .roar model saved to: {app_state.model_restore_path}")
        return output
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        if self.save_roar_on_train_end:
