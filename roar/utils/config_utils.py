...
def update_model_config(
    model_cls: 'roar.core.config.modelPT.NemoConfig', update_cfg: 'DictConfig', drop_missing_subconfigs: bool = True
):
    """
    ...
    Returns:
        A DictConfig with updated values that can be used to instantiate the Roar Model along with supporting
        infrastructure.
    """
    if not _HAS_HYDRA:
    ...
    # Remove ModelPT artifact `roar_version`
    if 'roar_version' in update_cfg.model:
        # Assume artifact from ModelPT and pop
        if 'roar_version' not in model_cls.model:
            with open_dict(update_cfg.model):
                update_cfg.model.pop('roar_version')
