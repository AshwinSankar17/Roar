...
class ClearMLLogger(Logger):
    ...
    def __init__(
        self, clearml_cfg: DictConfig, log_dir: str, prefix: str, save_best_model: bool, postfix: str = ".roar"
    ) -> None:
        ...
        self.path_roar_model = os.path.abspath(
        ...
        self.project_name = os.getenv("CLEARML_PROJECT", clearml_cfg.project if clearml_cfg.project else "Roar")
        ...
        if clearml_cfg.log_model:
            self.clearml_model: OutputModel = OutputModel(
                name=model_name, task=self.clearml_task, tags=tags, framework="Roar"
            )
    ...
    def after_save_checkpoint(self, checkpoint_callback: Checkpoint) -> None:
        if self.clearml_model:
            ...
            self._log_model(self.path_roar_model)
    ...
    def _log_model(self, save_path: str) -> None:
        if self.clearml_model:
            if os.path.exists(save_path):
                ...
                logging.warning((f"Logging model enabled, but cant find .roar file!" f" Path: {save_path}"))
