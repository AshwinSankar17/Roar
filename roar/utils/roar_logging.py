...
from roar.constants import ROAR_ENV_VARNAME_REDIRECT_LOGS_TO_STDERR, ROAR_ENV_VARNAME_TESTING
from roar.utils.env_var_parsing import get_envbool
from roar.utils.formatters.base import BaseRoarFormatter, DebugRoarFormatter
from roar.utils.get_rank import is_global_rank_zero
from roar.utils.metaclasses import Singleton
...
class Logger(metaclass=Singleton):
    ...
    def _define_logger(self, capture_warnings=True):
        ...
        with self._logger_lock:
            try:
                self._logger = _logging.getLogger("roar_logger")
                # By default, silence all loggers except the logger for rank 0
                self.remove_stream_handlers()
                # If ROAR_TESTING is set, add a streamhandler to all ranks
                if get_envbool(ROAR_ENV_VARNAME_TESTING, False):
                    ...
                    self.add_stream_handlers(formatter=DebugRoarFormatter)
    ...
    def add_stream_handlers(self, formatter=BaseRoarFormatter):
        """Add StreamHandler that log to stdout and stderr to the logger. INFO and lower logs are streamed to stdout
        while WARNING and higher are streamed to stderr. If the ROAR_ENV_VARNAME_REDIRECT_LOGS_TO_STDERR environment
        ...
        if get_envbool(ROAR_ENV_VARNAME_REDIRECT_LOGS_TO_STDERR, False):
    ...
    def reset_stream_handler(self, formatter=BaseRoarFormatter):
