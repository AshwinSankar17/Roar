...
from roar import constants
from roar.utils import logging


def resolve_cache_dir() -> pathlib.Path:
    """
    Utility method to resolve a cache directory for Roar that can be overriden by an environment variable.

    Example:
        ROAR_CACHE_DIR="~/roar_cache_dir/" python roar_example_script.py

    Returns:
        A Path object, resolved to the absolute path of the cache directory. If no override is provided,
        uses an inbuilt default which adapts to roar versions strings.
    """
    override_dir = os.environ.get(constants.ROAR_ENV_CACHE_DIR, "")
    if override_dir == "":
        path = pathlib.Path.joinpath(pathlib.Path.home(), f'.cache/torch/Roar/Roar_{ROAR_VERSION}')
    else:
...
def is_datastore_cache_shared() -> bool:
    ...
    cache_shared = int(os.environ.get(constants.ROAR_ENV_DATA_STORE_CACHE_SHARED, 1))
    ...
    if cache_shared == 0:
        ...
        raise ValueError(f'Unexpected value of env {constants.ROAR_ENV_DATA_STORE_CACHE_SHARED}')


def ais_cache_base() -> str:
    """Return path to local cache for AIS.
    """
    override_dir = os.environ.get(constants.ROAR_ENV_DATA_STORE_CACHE_DIR, "")
    if override_dir == "":
        cache_dir = resolve_cache_dir().as_posix()
    else:
        cache_dir = pathlib.Path(override_dir).resolve().as_posix()

    if cache_dir.endswith(ROAR_VERSION):
        # Prevent re-caching dataset after upgrading Roar
