...
def maybe_download_from_cloud(url, filename, subfolder=None, cache_dir=None, refresh_cache=False) -> str:
    ...
    Args:
        ...
        cache_dir: (str) a cache directory where to download. If not present, this function will attempt to create it.
            If None (default), then it will be $HOME/.cache/torch/Roar
    ...
    if cache_dir is None:
        cache_location = Path.joinpath(Path.home(), ".cache/torch/Roar")
    else:
...
def initialize_sagemaker() -> None:
    """
    Helper function to initiate sagemaker with Roar.
    This function installs libraries that Roar requires for the ASR toolkit + initializes sagemaker ddp.
    """
